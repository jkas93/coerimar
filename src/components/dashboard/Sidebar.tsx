  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center border-b border-accent-400/10 transition-all duration-300 overflow-hidden ${isCollapsed ? 'p-6 justify-center' : 'p-6 justify-start'}`}>
        <Link href="/dashboard" className="flex items-center gap-3 w-full" title={isCollapsed ? "COERIMAR" : ""}>
          ...
          {!isCollapsed && <span className="font-bold text-lg gradient-text whitespace-nowrap fade-in-fast uppercase tracking-wider">COERIMAR</span>}
        </Link>
      </div>
      ...
      <nav className={`flex-1 overflow-x-hidden space-y-1 transition-all duration-300 ${isCollapsed ? 'p-3 flex flex-col items-center' : 'p-4'}`}>
        ...
        {profile?.is_superadmin && (
          <Link
            ...
            className={`flex items-center gap-3 rounded-lg text-sm font-medium transition-colors ${
               pathname === '/users' ? 'text-accent-400 bg-accent-400/10' : 'text-slate-300 hover:text-accent-400 hover:bg-accent-400/10'
            } ${isCollapsed ? 'p-3 justify-center' : 'px-4 py-2.5 w-full'}`}
          >
            <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
            </svg>
            {!isCollapsed && <span className="whitespace-nowrap fade-in-fast">Personal</span>}
          </Link>
        )}
      </nav>
